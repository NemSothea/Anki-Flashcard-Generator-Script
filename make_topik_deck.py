...
INPUT = "topik_words.csv"
OUTPUT = "topik2_deck.apkg"

# Fixed IDs (not randomized) so re-running this script updates the same
# Anki deck/model on reimport instead of creating a duplicate every day.
MODEL_ID = 1607392319
DECK_ID = 1607392320
...
def create_deck():
    """Create the TOPIK II Anki deck from CSV input"""

    my_model = genanki.Model(
        MODEL_ID,
        "TopikIIVocabularyModel",
        fields=[
            {"name": "Word"},
            {"name": "pos"},
            {"name": "definition"},
    ...
    deck = genanki.Deck(DECK_ID, "TOPIK II Vocabulary")
