...
3. Every image directly inside `Topik II Vocabulary List Resource/` (not already in its `processed/` subfolder) converted, transcribed, and appended as new rows to `topik_words.csv`, then moved into `Topik II Vocabulary List Resource/processed/`.
...
2. For each new image: if it's `.HEIC`, convert it to JPEG with `sips -s format jpeg "<file>" --out "Topik II Vocabulary List Resource/processed/<name>.jpg"` (keep the jpg — don't delete it after). If it's already JPG/PNG, just view it directly.
...
5. After all entries from an image are transcribed and appended, move that image's original file (the `.HEIC` or source file) into `Topik II Vocabulary List Resource/processed/`. Never delete a source image.
...
<stop_conditions>
Stop and ask before: deleting any file, installing any dependency, or touching any file outside the scope lock above. Moving source images into `processed/` after successful transcription is expected and does not need confirmation.
</stop_conditions>
